...
export default function MemeGeneratorPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        console.error('API Error:', data.message || 'API 请求失败，请检查后端服务。');
  ...
  console.log('Rendering MemeGeneratorPage component. Credits:', credits);
  ...
  return (
    <div className={styles.container}>
      ...
      <main className={styles.main}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%', maxWidth: '400px', marginBottom: '2rem' }}>
          <h1 className={styles.title} style={{ fontSize: '2rem', margin: 0 }}>AI 梗图生成器</h1>
          <Link href="/" className={styles.downloadLink} style={{ backgroundColor: '#555', padding: '0.5rem 1rem', borderRadius: '5px', color: 'white' }}>
            返回主页
          </Link>
        </div>
